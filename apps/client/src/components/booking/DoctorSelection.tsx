...
import { listDoctorsBySpeciality } from '../../api/doctor';
...
export default function DoctorSelection({ speciality, onNext, onBack }: DoctorSelectionProps) {
    ...
    console.log("Speciality", speciality?.name);

    const { data: doctorsData, isPending: doctorsPending } = useQuery({
        queryKey: ['doctors', speciality?.name],
        queryFn: () => listDoctorsBySpeciality({ speciality: speciality?.name }),
    });
